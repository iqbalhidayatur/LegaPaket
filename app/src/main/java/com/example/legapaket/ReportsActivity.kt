...
class ReportsActivity : AppCompatActivity() {
    ...
    private val canEdit: Boolean get() = UserSession.isAdmin
    ...
    private fun setupFilterBar() {
        ...
        spinnerCity.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p: AdapterView<*>, v: View?, pos: Int, id: Long) {
                filterCity = cities[pos]; applyFilterAndSort(); adapter.notifyDataSetChanged(); updateSummary()
            }
        ...
        spinnerSort.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p: AdapterView<*>, v: View?, pos: Int, id: Long) {
                sortMode = sortOptions[pos]; applyFilterAndSort(); adapter.notifyDataSetChanged(); updateSummary()
            }
            override fun onNothingSelected(p: AdapterView<*>) {}
        }
        ...
        btnResetFilter.setOnClickListener {
            ...
            applyFilterAndSort(); adapter.notifyDataSetChanged(); updateSummary()
        }
    }
    ...
    private fun loadData() {
        allData.clear()
        ShipmentRepository.getAll().forEach {
            allData.add(ReportModel(
                resi          = it.resi,
    ...
    private fun applyFilterAndSort() {
        ...
        if (filterCity != "Semua Kota") filtered = filtered.filter { it.city == filterCity }
    ...
    private fun setupNavigation() {
        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.menu_dashboard -> {
                    if (canEdit) startActivity(Intent(this, DashboardActivity::class.java))
                    else Toast.makeText(this, "Akses tidak diizinkan", Toast.LENGTH_SHORT).show()
                    canEdit
                }
                R.id.menu_shipment -> {
                    if (canEdit) startActivity(Intent(this, ShipmentActivity::class.java))
                    else Toast.makeText(this, "Akses tidak diizinkan", Toast.LENGTH_SHORT).show()
                    canEdit
                }
